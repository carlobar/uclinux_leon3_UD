...
#define MACADDR0 CONFIG_GRLIB_OPENCORES_MACADDR0
#define MACADDR1 CONFIG_GRLIB_OPENCORES_MACADDR1
#define MACADDR2 CONFIG_GRLIB_OPENCORES_MACADDR2
#define MACADDR3 CONFIG_GRLIB_OPENCORES_MACADDR3
#define MACADDR4 CONFIG_GRLIB_OPENCORES_MACADDR4
#define MACADDR5 CONFIG_GRLIB_OPENCORES_MACADDR5
#define IRQ_ETH_0 CONFIG_GRLIB_OPENCORES_IRQ_ETH_0
...
oeth_rx(struct net_device *dev)
{
	...
	for (;;cep->rx_cur = (cep->rx_cur + 1) & OETH_RXBD_NUM_MASK) {
      ...
      bdp->len_status |= OETH_RX_BD_EMPTY;
		...
		skb = alloc_skb(pkt_len + 4, GFP_ATOMIC);
 ...
 */
int opencores_ethermac_probe(struct net_device *dev)
{
	struct oeth_private *cep;
	volatile oeth_regs *regs;
	...
	printk("Attaching grlib openeth ethernet driver:\n");
...
#ifdef CONFIG_GRLIB_OPENCORES_ETHERNET_MII_100MB
  // switch to 100 mbit ethernet
  regs->miiaddress = 0;
  regs->miitx_data = 0x2000;
