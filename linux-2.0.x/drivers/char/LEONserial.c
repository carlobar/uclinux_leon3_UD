...
#define LEON_CLOCK ((unsigned long)(((leon_regs->scalerload) + 1)*(unsigned long)1000*(unsigned long)1000))
...
static int get_baud_id() {
  int sc = leon_regs->uartscaler1;
...
void rs_put_char(char ch)
{
        ...
        save_flags(flags); cli();
	loops = 0;
	while (!(leon_regs->uartstatus1 & USTAT_TH) && (loops < 100000))
        	loops++;

	leon_regs->uartdata1 = ch;

	loops = 0;
	while (!(leon_regs->uartstatus1 & USTAT_TS) && (loops < 100000))
        	loops++;
        restore_flags(flags);
}
...
static void rs_start(struct tty_struct *tty)
{
	...
	save_flags(flags); cli();
	if (info->xmit_cnt && info->xmit_buf && !(leon_regs->uartctrl1 & UCTRL_TE))
		leon_regs->uartctrl1 |= UCTRL_TE | UCTRL_TI;
	restore_flags(flags);
}
...
static _INLINE_ void receive_chars(struct LEON_serial *info, struct pt_regs *regs, unsigned short rx)
{
	...
	} while((rx = leon_regs->uartstatus1) & USTAT_DR);
...
static _INLINE_ void transmit_chars(struct LEON_serial *info)
{
	if (info->x_char) {
		/* Send next char */
		leon_regs->uartdata1 = info->x_char;
		info->x_char = 0;
	...
	if((info->xmit_cnt <= 0) || info->tty->stopped) {
		...
		leon_regs->uartctrl1 &= ~UCTRL_TI;
		goto clear_and_return;
	}

	/* Send char */
	leon_regs->uartdata1 = info->xmit_buf[info->xmit_tail++];
...
void rs_interrupt(int irq, void *dev_id, struct pt_regs * regs)
{
	...
	unsigned int stat = leon_regs->uartstatus1;
	/* clear all error flags; others remain unchanged */
	leon_regs->uartstatus1 = 0;
...
static int startup(struct LEON_serial * info)
{
	...
	leon_regs->uartctrl1 |= UCTRL_RE | UCTRL_RI | UCTRL_TE /*| UCTRL_TI*/;
	//leon_regs->uartdata1;
...
static void shutdown(struct LEON_serial * info)
{
	...
	leon_regs->uartctrl1 &= ~(UCTRL_RE | UCTRL_RI | UCTRL_TE); /* All off! */
	if (!(info->flags & S_INITIALIZED))
		return;
...
void change_speed(struct LEON_serial *info)
{
	...
	printk ("baud_table[i]:  %i\n",baud_table[i]);
	printk ("Leonclock: %i\n",LEON_CLOCK);
	printk ("comine: %i\n",scale);
	printk ("     \n");
	
	ustcnt = leon_regs->uartctrl1;
	leon_regs->uartctrl1 = ustcnt & ~UCTRL_TE;

	info->baud = baud_table[i];
	//note: bootloader initialized timer scalar reload 
	leon_regs->uartscaler1 = scale; 
		    ...
		    const unsigned char *buf, int count)
{
	...
	while (info->xmit_cnt > 0) {
		if (leon_regs->uartstatus1 & USTAT_TH) {
			leon_regs->uartdata1 = info->xmit_buf[info->xmit_tail++];
			info->xmit_tail = info->xmit_tail & (SERIAL_XMIT_SIZE-1);
			info->xmit_cnt--;
		}
...
static void rs_close(struct tty_struct *tty, struct file * filp)
{
	...
	leon_regs->uartctrl1 &= ~(UCTRL_RE | UCTRL_RI);
...
int rsLEON_init(void)
{
	...
	serial_driver.init_termios.c_cflag = get_baud_id();
