...
#if defined(CONFIG_LEON_3) || defined(CONFIG_LEON)

#include <asm/leon.h>

extern volatile LEON3_VGA_Regs_Map *leon_apbvga;

#define dbg2_printf(fmt,arg...) \
                {\
                  char b[1000];\
                  sprintf(b,fmt,##arg);\
                  console_print_LEON(b);\
                }
...
#define _DBG_FUNC //dbg_printf("%s\n",__FUNCTION__);
#define _DBG_FUNC_START //dbg_printf("%s-start: \n",__FUNCTION__);
#define _DBG_FUNC_END //dbg_printf("%s-end: \n",__FUNCTION__);

#define IS_VIDEO_MEMORY(a) \
        ((unsigned long)(a) - video_mem_base < video_mem_term - video_mem_base)
#define LEON_VGAMEM_MEM2OFF(a) (((unsigned long)(a)-video_mem_base)/2)
#define LEON_VGAMEM_WRITE(off,v) do {  \
          if (leon_apbvga != 0) { \
            leon_apbvga->write = (off<<8) | (v & 0xff); \
          } \
        } while(0)
...
#endif /* defined(CONFIG_LEON_3) || defined(CONFIG_LEON) */
#endif /* __alpha__ */
