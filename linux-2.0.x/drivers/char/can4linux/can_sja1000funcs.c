 ...
 *------------------------------------------------------------------
 * $Header: /cvs/sw/linux-2.4.x/drivers/char/can4linux/can_sja1000funcs.c,v 1.1.1.2 2003/08/29 01:04:37 davidm Exp $
 *
 *--------------------------------------------------------------------------
 *
 *
...
# define canmode canctl
    ...
    CANout(board, canclk, CAN_MODE_PELICAN + CAN_MODE_CLK);
     DBGprint(DBG_DATA, ("[%d] CAN_clk 0x%x\n", board, CANin(board, canclk)));
...
int custom=0;
    ...
    switch(baud)
    {
      //case   10: i = 0; break; //10KBaud can't be reached with fclk=40MHz
	case   20: i = 1; break;
	case   40: i = 2; break;
	case   50: i = 3; break;
	case  100: i = 4; break;
    ...
    //set fix baudrate of 500kbaud only for tests
    //CANout(board,cantim0, (u8) 0x83);
    //CANout(board,cantim1, (u8) 0x25);
...
u8 tx2reg, stat;
    ...
    DBGprint(DBG_DATA, ("Send Message\n"));
...
/* both drivers use high memory area */
    ...
    if( check_region(Base[minor], CAN_RANGE ) ) {
          ...
          DBGprint(DBG_DATA,("Request Region IO **\n"));

#else
          request_region(Base[minor], can_range[minor] );
          DBGprint(DBG_DATA,("Request Region **\n"));
...
/* End: 2. Vendor specific part ------------------------------------------- */
    ...
    // Hack hobe
    printk("Alle irqs löschen\n");
        
    printk("Irq register  = 0x%X\n",CANin(0, canirq));
    printk("Irq enable    = 0x%X\n",CANin(0, canirq_enable));
...
Tx_done:
 ...
 } while( (irqsrc = CANin(minor, canirq)) != 0);                             // Hack
