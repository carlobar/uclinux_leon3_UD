 ...
 *------------------------------------------------------------------
 * $Header: /cvs/sw/linux-2.4.x/drivers/char/can4linux/can_defs.h,v 1.1.1.2 2003/08/29 01:04:37 davidm Exp $
 *
 *--------------------------------------------------------------------------
 *
 *
...
#define CANout(bd,adr,v)	\
             ((canregs_t *)can_base[bd])->adr = v

#define CANset(bd,adr,m)	\
              ((canregs_t *)can_base[bd])->adr = (((canregs_t *)can_base[bd])->adr | (m))

#define CANreset(bd,adr,m)	\
              (((canregs_t *)can_base[bd])->adr = ((canregs_t *)can_base[bd])->adr & ~(m))

#define CANin(bd,adr)		\
              ((canregs_t *)can_base[bd])->adr  

#define CANtest(bd,adr,m)	\
              (((canregs_t *)can_base[bd])->adr & (m))
