...
*/
//#define SMC_DEBUG 4 // Must be defined in makefile
#define SMC_DEBUG 0 // Must be defined in makefile

/* &1: register dump */
/* &2: rx packet dump */
/* &4: tx packet dump */
//#define SMC_DEBUG_BIT 0x7fff
#define SMC_DEBUG_BIT 0x0
...
#elif defined(CONFIG_LEON_2) || defined(CONFIG_LEON_3)
#include <asm/sparcsmc.h>
#include <asm/leon.h>
#define insl insl_align2
#else
#include <asm/io.h>
#endif
...
#elif defined(CONFIG_LEON_2) || defined(CONFIG_LEON_3)
static unsigned int smc_portlist[] = { CONFIG_SMC91111_BASE_ADD, 0 }; 
static unsigned int smc_irqlist[]  = { CONFIG_SMC91111_IRQ, 0 };
#else
...
#define MEMORY_WAIT_TIME_LOOP 1024*1024
...
static void smc_hardware_send_packet( struct device * dev, int );
...
static void smc_enable( struct device *dev )
{
	...
	SMC_SELECT_BANK( 2 );
#if defined(CONFIG_SMC16BITONLY)
        outw( (SMC_INTERRUPT_MASK)|IM_TX_EMPTY_INT|IM_TX_INT, ioaddr + INT_REG ); //accnowledge old
        outw( (SMC_INTERRUPT_MASK << 8), ioaddr + INT_REG );
#else
	outb( SMC_INTERRUPT_MASK|IM_TX_EMPTY_INT|IM_TX_INT, ioaddr + INT_REG ); //accnowledge old
...
{
    struct          smc_local *lp   = (struct smc_local *)dev->priv;
    unsigned long   tcr, ioaddr          = dev->base_addr;
    word            length;
    unsigned short  numPages, packet_no;
    word            time_out;
    word            status;
    ...
    /* check if transmit was disabled, reenable */
    SMC_SELECT_BANK( 2 );
    tcr = inw( ioaddr + TCR_REG);
    if (0 == (TCR_ENABLE & tcr)) {
      outw( tcr | TCR_ENABLE, ioaddr + TCR_REG );
    }
    ...
    do {
      ...
      time_out = MEMORY_WAIT_TIME_LOOP;
      do  {
      ...
      if ( packet_no & AR_FAILED ) {
          PRINTK2("%s: alloc failed retrying ...  \n", dev->name);
          continue;
      }
    } while (0);
...
/*     } */
    ...
    // send the packet now.
    smc_hardware_send_packet(dev,packet_no);
...
*/
static void smc_hardware_send_packet( struct device * dev, int packet_no )
{
	struct smc_local *lp = (struct smc_local *)dev->priv;
	//byte	 		packet_no;
...
/* 	} */
   	...
   	PRINTK3("%s: Trying to xmit packet(%d) of length 0x%x\n",
		dev->name, packet_no, length);

#if SMC_DEBUG_BIT & 4
	printk("%s:<<<<<<<<<<<<<<Transmitting Packet\n",dev->name);
...
#if defined(CONFIG_SMC16BITONLY) || (defined(CONFIG_LEON_2) || defined(CONFIG_LEON_3))
	...
	outw((0),ioaddr + DATA_REG);        /* reserve space for status word*/
        outw((0x7FE & (length + 6)),ioaddr + DATA_REG ); /* Always even, always < 15xx(dec)*/
#elif defined(CONFIG_SMC91C111_32_BIT)
#if defined(CONFIG_SMC16BITONLY)
	...
	*/
...
#if (defined(CONFIG_LEON_2) || defined(CONFIG_LEON_3))
        {
          ...
          unsigned short *sdata = (unsigned short *)buf;
          int len = length;
          while(len >= sizeof(*sdata)) {
            ((*(volatile unsigned short *) (ioaddr+DATA_REG)) = ((*sdata++)));
            len -= 2;
          }
          ...
          outw((control),ioaddr+DATA_REG);
        }
        
#elif defined(CONFIG_SED_SIOS)
	...
	if ( (length & 1) == 0 ) {
        ...
        /* accnowledge interrupts */
        SMC_SELECT_BANK(2);
        outw(inw(ioaddr + INT_REG)|IM_TX_EMPTY_INT|IM_TX_INT,ioaddr + INT_REG); 
        
	/* enable the interrupts */
	SMC_ENABLE_INT( (IM_TX_INT ) ); /* eror only (AUTORELEASE) */
        ...
        *((volatile unsigned int *) 0x800000A8) |= 
        ...
        if ((addr_pio = amba_find_apbslv_addr(VENDOR_GAISLER, GAISLER_PIOPORT, &irq_pio)) &&
            ...
            *((volatile unsigned int *) addr_mctrl) |= 0x10f80000; //mctrl ctrl 1
            io->irqmask |= (1 << CONFIG_SMC91111_PIO);
            io->irqpol  |= (1 << CONFIG_SMC91111_PIO);
            io->irqedge |= (1 << CONFIG_SMC91111_PIO);
            io->iodir &= ~(1 << CONFIG_SMC91111_PIO);
        } else {
          return -ENXIO;
        }
#endif
...
static int smc_probe( unsigned long ioaddr )
{
	...
	printk("LAN91CXX(0x%x) - type: %01x, rev: %01x\n",revision_register,
                (revision_register>>4)&0xf, revision_register & 0xf);
...
/* Set a default MAC address */
	SMC_SELECT_BANK( 1 );
	outw(_swapw(0x1213), ioaddr + ADDR0_REG);
	outw(_swapw(0x1415), ioaddr + ADDR0_REG + 2);
	outw(_swapw(0x1617), ioaddr + ADDR0_REG + 4);
...
static int smc_send_packet(struct sk_buff *skb, struct device *dev)
{
  unsigned long status, ioaddr;
  ioaddr = dev->base_addr;
  
	PRINTK3("%s:smc_send_packet\n", dev->name);

        /* some irq's get swallowed, do a polling here*/
	/* SMC_SELECT_BANK(2); */
/*         status = inb( ioaddr + INT_REG ); */
...
static void smc_interrupt(int irq, void * dev_id,  struct pt_regs * regs)
{
	...
	*((volatile unsigned long *) (MCF_MBAR2+MCFSIM2_GPIOINTCLEAR)) =
	...
	do
    {
        ...
        if (timeout == 0) {
          PRINTK3("%s: bailing out because of timeout\n", dev->name);
        }
...
static void smc_rcv(struct device *dev)
{
...
#if defined(CONFIG_SED_SIOS) || defined(CONFIG_LEON_2) || defined(CONFIG_LEON_3)
    {
    ...
    rlen = packet_length - 6;
    packet_length = (packet_length & 0xfffc) + 4;
    packet_length &= 0x07ff;  // mask off top bits

    PRINTK2("%s:receive  status 0x%04x length field %04d(%04d) length %04d(&0x07ff)\n",
            dev->name, status, rlen, packet_length, (packet_length&0x07ff));
...
void smc_register_dump(const struct device *dev)
{
    ...
    PRINTK("%s:Register Dump Bank 0\n", dev->name);
        PRINTK("\t%s:[TCR_REG]        Register 0x%02x: 0x%04x\n", dev->name,
                0, (inw(ioaddr + 0)));
        PRINTK("\t%s:[EPH_STATUS_REG] Register 0x%02x: 0x%04x\n", dev->name,
                2, (inw(ioaddr + 2)));
        PRINTK("\t%s:[RCR_REG]        Register 0x%02x: 0x%04x\n", dev->name,
                4, (inw(ioaddr + 4)));
        PRINTK("\t%s:[COUNTER_REG]    Register 0x%02x: 0x%04x\n", dev->name,
                6, (inw(ioaddr + 6)));
        PRINTK("\t%s:[MIR_REG]        Register 0x%02x: 0x%04x\n", dev->name,
                8, (inw(ioaddr + 8)));
        PRINTK("\t%s:[RPC_REG]        Register 0x%02x: 0x%04x\n", dev->name,
                0xa, (inw(ioaddr + 0xa)));
        PRINTK("\t%s:[BSR_REG]        Register 0x%02x: 0x%04x\n", dev->name,
                0xc, (inw(ioaddr + 0xc)));
    
    SMC_SELECT_BANK(1);
    PRINTK("%s:Register Dump Bank 1\n", dev->name);
        PRINTK("\t%s:[CONFIG_REG]     Register 0x%02x: 0x%04x\n", dev->name,
                0, inw(ioaddr + 0));
        PRINTK("\t%s:[BASE_REG]       Register 0x%02x: 0x%04x\n", dev->name,
                2, inw(ioaddr + 2));
        PRINTK("\t%s:[ADDR0_REG]      Register 0x%02x: 0x%04x\n", dev->name,
                4, inw(ioaddr + 4));
        PRINTK("\t%s:[ADDR1_REG]      Register 0x%02x: 0x%04x\n", dev->name,
                6, inw(ioaddr + 6));
        PRINTK("\t%s:[ADDR2_REG]      Register 0x%02x: 0x%04x\n", dev->name,
                8, inw(ioaddr + 8));
        PRINTK("\t%s:[GP_REG]         Register 0x%02x: 0x%04x\n", dev->name,
                0xa, inw(ioaddr + 0xa));
        PRINTK("\t%s:[CTL_REG]        Register 0x%02x: 0x%04x\n", dev->name,
                0xc, inw(ioaddr + 0xc));

    SMC_SELECT_BANK(2);
    PRINTK("%s:Register Dump Bank 2\n", dev->name);
        PRINTK("\t%s:[MMU_CMD_REG]    Register 0x%02x: 0x%04x\n", dev->name,
                0, inw(ioaddr + 0));
        PRINTK("\t%s:[PN_REG|AR_REG]  Register 0x%02x: 0x%04x\n", dev->name,
                2, inw(ioaddr + 2));
        PRINTK("\t%s:[RX|TXFIFO_REG]  Register 0x%02x: 0x%04x\n", dev->name,
                4, inw(ioaddr + 4));
        PRINTK("\t%s:[PTR_REG]        Register 0x%02x: 0x%04x\n", dev->name,
                6, inw(ioaddr + 6));
        PRINTK("\t%s:[DATA_REG]       Register 0x%02x: 0x%04x\n", dev->name,
                8, inw(ioaddr + 8));
        PRINTK("\t%s:[M|INT_REG]      Register 0x%02x: 0x%04x\n", dev->name,
                0xc, inw(ioaddr + 0xc));
    
    SMC_SELECT_BANK(3);
    PRINTK("%s:Register Dump Bank 3\n", dev->name);
        PRINTK("\t%s:[MCAST_REG1]     Register 0x%02x: 0x%04x\n", dev->name,
                0, inw(ioaddr + 0));
        PRINTK("\t%s:[MCAST_REG2]     Register 0x%02x: 0x%04x\n", dev->name,
                2, inw(ioaddr + 2));
        PRINTK("\t%s:[MCAST_REG3]     Register 0x%02x: 0x%04x\n", dev->name,
                4, inw(ioaddr + 4));
        PRINTK("\t%s:[MCAST_REG4]     Register 0x%02x: 0x%04x\n", dev->name,
                6, inw(ioaddr + 6));
        PRINTK("\t%s:[MII_REG]        Register 0x%02x: 0x%04x\n", dev->name,
                8, inw(ioaddr + 8));
        PRINTK("\t%s:[REV_REG]        Register 0x%02x: 0x%04x\n", dev->name,
                0xa, inw(ioaddr + 0xa));
