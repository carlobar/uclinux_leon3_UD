...
static int do_load_script(struct linux_binprm *bprm,struct pt_regs *regs)
{
	...
	*cp = '\0';
	...
	printk("Returned from binary\n");
	return retval;
#else /* !NO_MM */
	{
		char ** oldargv = bprm->argv;
		int oldargc = bprm->argc;
		int i,j = strlen(i_name);
		char *k;
		
		bprm->argc = oldargc + (i_arg ? 2 : 1);
		bprm->argv = kmalloc(sizeof(char*) * bprm->argc + j+1, GFP_KERNEL);
		...
		k = ((char *)(bprm->argv))+(sizeof(char*) * bprm->argc);
		memcpy(k,i_name,j+1);
		i_name = k;
