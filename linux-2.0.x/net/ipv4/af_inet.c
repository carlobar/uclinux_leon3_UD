...
static struct proc_dir_entry proc_rarp = {
...
static struct proc_dir_entry proc_raw = {
		PROC_NET_RAW, 3, "raw",
		S_IFREG | S_IRUGO, 1, 0, 0,
		0, &proc_net_inode_operations,
		raw_get_info
...
static struct proc_dir_entry proc_snmp = {
		PROC_NET_SNMP, 4, "snmp",
		S_IFREG | S_IRUGO, 1, 0, 0,
		0, &proc_net_inode_operations,
		snmp_get_info
	};
static struct proc_dir_entry proc_sockstat = {
...
static struct proc_dir_entry proc_route = {
		PROC_NET_ROUTE, 5, "route",
		S_IFREG | S_IRUGO, 1, 0, 0,
		0, &proc_net_inode_operations,
		rt_get_info
	};
static struct proc_dir_entry proc_rt_cache = {
...
#ifdef CONFIG_INET_RARP
	proc_net_register(&proc_rarp);
#endif		/* RARP */

	proc_net_register(&proc_raw);
	proc_net_register(&proc_snmp);
	proc_net_register(&proc_sockstat);
	proc_net_register(&proc_tcp);
	proc_net_register(&proc_udp);
	proc_net_register(&proc_route);
	proc_net_register(&proc_rt_cache);
#endif		/* CONFIG_PROC_FS */
}
