...
struct signal_sframe {
	struct dummy_reg_window sig_window;
	int sig_num;
	int sig_code;
	...
	if(current->tss.w_saved)
		for(window = 0; window < current->tss.w_saved; window++) {
			...
			memcpy(&sc->sigc_wbuf[window],
			       ...
			       sizeof(struct dummy_reg_window));
		}
	else
		memcpy(sframep, (char *)regs->u_regs[UREG_FP],
		       sizeof(struct dummy_reg_window));

	sframep->retcode[0] = 0x82102077;	/* mov 119, %g1 ; sys_sigreturn */
	sframep->retcode[1] = 0x91d02010;	/* ta 0x10 ; syscall */
	regs->u_regs[UREG_I7] = ((unsigned long) &sframep->retcode) - 8;	/* Set new return addr */
