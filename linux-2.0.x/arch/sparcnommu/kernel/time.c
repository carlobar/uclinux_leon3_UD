...
void time_init(void)
{
	...
	request_irq(IRQ8, timer_interrupt, IRQ_FLG_LOCK, "timer", NULL);
#else
	request_irq(LEON3_GpTimer_Irq, timer_interrupt, IRQ_FLG_LOCK, "timer", NULL);
#endif        
	//regs->scalercnt  = 33;
	//regs->scalerload = 33;
	//regs->timercnt1  = 1999;
	//regs->timerload1 = 1999; /* HZ = 100 */
	//regs->timerctrl1 = 0x07; /* Load, auto reload, enable */
}
