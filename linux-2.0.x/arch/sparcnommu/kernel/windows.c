...
static inline void shift_window_buffer(int first_win, int last_win, struct thread_struct *tp)
{
	...
	for(i = first_win; i < last_win; i++) {
		...
		memcpy(&tp->reg_window[i], &tp->reg_window[i+1], sizeof(struct dummy_reg_window));
	}
}
...
void synchronize_user_stack(void)
{
	...
	for(window = tp->w_saved - 1; window >= 0; window--) {
		...
		if(verify_area(VERIFY_WRITE, (char *) sp, sizeof(struct dummy_reg_window)))
			continue;

		/* Ok, let it rip. */
		memcpy((char *) sp, &tp->reg_window[window], sizeof(struct dummy_reg_window));
...
#define stack_is_bad(sp, rw) \
  (((sp) & 7) || verify_area(rw, (char *) (sp), sizeof(struct dummy_reg_window)))
...
void try_to_clear_window_buffer(struct pt_regs *regs, int who)
{
	...
	flush_user_windows();
#if 0
printk("%s(%d): tp->w_saved=%d\n", __FILE__, __LINE__, tp->w_saved);
#endif
