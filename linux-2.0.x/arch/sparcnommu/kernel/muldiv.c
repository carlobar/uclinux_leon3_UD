...
int do_emul_y(struct pt_regs *regs, unsigned long pc) {

  unsigned int insn;
  unsigned int rs1, rs2, rdv;
  if (get_user (insn, (unsigned int *)pc)) return;
  
  rs1 = (insn >> 14) & 0x1f;
  rs2 = insn & 0x1f;
  rdv = (insn >> 25) & 0x1f;
  
  //leon umc hardware bug: mv %y,reg
  if ((insn &0xc1f80000) == 0x81400000) {
    unsigned long y;
    __asm__ __volatile__ ("\n\t"
			  "mov	%y, %0\n\t"
			  : "=r" (y)
			  :
			  : );
    if (store_reg(y, rdv, regs))
      return -1;
  }
  //leon umc hardware bug: mv reg,%y
  else if ((insn &0xc1f80000) == 0x81800000) {
